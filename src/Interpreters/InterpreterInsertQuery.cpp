...
QueryPipeline InterpreterInsertQuery::buildInsertSelectPipeline()
{
    ...
    pipeline.resize(1);
    pipeline.addSimpleTransform([&](const Block &in_header) -> ProcessorPtr {
        return std::make_shared<NumberBlocksTransform>(in_header);
    });
...
QueryPipeline InterpreterInsertQuery::buildInsertPipeline()
{
    ...
    chain.addSource(std::make_shared<NumberBlocksTransform>(chain.getInputHeader()));
...
void registerInterpreterInsertQuery(InterpreterFactory & factory)
{
    ...
    {
        return std::make_unique<InterpreterInsertQuery>(args.query, args.context, args.allow_materialized, false, false, false);
    };
