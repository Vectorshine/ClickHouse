...
void AggregatingInOrderTransform::generate()
{
    ...
    to_push_chunk.addChunkInfo(std::make_shared<ChunkInfoWithAllocatedBytes>(cur_block_bytes));
...
void FinalizeAggregatedTransform::transform(Chunk & chunk)
{
    ...
    else if (!chunk.getChunkInfo<AggregatedChunkInfo>())
    {
        chunk.addChunkInfo(std::make_shared<AggregatedChunkInfo>());
    }
}
